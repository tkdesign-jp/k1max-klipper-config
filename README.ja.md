...
| サーミスタ | NTC100K B3950(CHCB-OT キット付属) | Klipper の `sensor_type = "Generic 3950"`。なお純正 K1 MAX cfg は同じコネクタに対して `EPCOS 100K B57560G104F` を指定しており、ベータ値がわずかに異なる。純正 cfg から移行する場合は `sensor_type` を `Generic 3950` に変更し、`PID_CALIBRATE EXTRUDER` を再実行すること。 |

設定値への影響:

- **センサータイプの表記**: この構成は NTC100K B3950。Klipper の正しい sensor_type は `Generic 3950`。Creality 純正 K1 MAX cfg は `EPCOS 100K B57560G104F` を指定していることが多く、ベータ値が微妙に異なる — 表記が合わないと高温域で数℃ずれる
...
```
printer.cfg          Klipper メイン設定(ヘッダにハードウェア構成を記載)
sensorless.cfg       センサーレスホーミング設定(printer.cfg から include)
gcode_macro.cfg      START_PRINT / END_PRINT 等のマクロ
printer_params.cfg   プリンタ全体パラメータ(kinematics, limits)
```

同梱の `printer.cfg` に関する注意:

- Creality が自動生成する `SAVE_CONFIG` ブロック(PID 保存値、bed mesh、prtouch デフォルト等)は **削除済み**。これらは個体ごとの校正結果なので他機に流用できない。自分でキャリブレーションを実行すれば Klipper が再生成する
- 元ファイルの `[prtouch_v2]` セクションには Creality 独自のバイナリ/エンコードされたデータが数行含まれている。これらは出荷時のまま保持してある — **編集しないこと**
- 元 cfg にあった `[include Helper-Script/...]` は上流の [Helper-Script](https://github.com/Guilouz/Creality-Helper-Script) プロジェクトを参照しており、このリポジトリには複製していない。Helper-Script を使う場合は別途インストールすること
